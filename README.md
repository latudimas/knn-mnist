...
The implementation of KNN with PCA using k=5 on the MNIST dataset shows interesting results across different numbers of PCA components. The 200-component model stands out as the best performer, achieving the highest accuracy of 93.50% and similarly high scores in precision, recall, and F1-score. This configuration strikes an optimal balance between performance and dimensionality reduction. The 10-component model, while less accurate at 89.20%, offers the fastest prediction time at 25.46 seconds, showcasing the trade-off between speed and accuracy. Interestingly, increasing to 500 components leads to a slight decrease in performance and the slowest prediction time of 29.02 seconds, suggesting that too many components may introduce unnecessary complexity. The results indicate that PCA effectively captures the essential features of the MNIST dataset, allowing for significant dimensionality reduction without sacrificing much performance. The 200-component model, with its high accuracy and moderate prediction time of 26.99 seconds, appears to be the best overall choice, offering a good balance between accuracy and computational efficiency. These findings highlight the importance of carefully selecting the number of PCA components to optimize both performance and speed in machine learning applications.
